...
def service_rate_to_dict(s: ServiceRate):
    return {
        ...
        'after_hours_multiplier': float(s.after_hours_multiplier) if s.after_hours_multiplier is not None else None,
        'weekend_multiplier': float(s.weekend_multiplier) if s.weekend_multiplier is not None else None,
...
def service_rate_update(request, pk):
    ...
    s.after_hours_multiplier = request.POST.get('after_hours_multiplier') or s.after_hours_multiplier
    s.weekend_multiplier = request.POST.get('weekend_multiplier') or s.weekend_multiplier
