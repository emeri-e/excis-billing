...
(function RateCardApp() {
    ...
    function renderDefault() {
      ...
      <tr data-id="${r.id}">
        ...
        <td>${r.status}</td>
        <td class="actions">
          <button class="iconbtn rc-view" data-id="${r.id}" data-viewtype="details">👁️</button>
          <button class="iconbtn rc-edit" data-id="${r.id}">✏️</button>
          <button class="iconbtn rc-del" data-id="${r.id}">🗑️</button>
        </td>
      </tr>`).join('');
    ...
    function renderServiceRateCard() {
        ...
        const subHeaders = categoryOrder.map(cat => {
            const types = Array.from(categoryTypes[cat] || []);
            if (types.length === 0) return `<th>—</th>`; // placeholder if no types discovered
        ...
        const rowsHtml = rowsData.map((r, idx) => {
            ...
            svc.forEach(s => {
                ...
                // if multiple entries exist for same cat+type, prefer the first — change to aggregation if needed
            ...
            const cells = categoryOrder.map(cat => {
                ...
                if (types.length === 0) {
                    ...
                    const val = s ? String(s.rate_value ?? '—') : '—';
        ...
        <td>${esc(r.status)}</td>
        <td class="actions">
          <button class="iconbtn rc-view" data-id="${r.id}">👁️</button>
          <button class="iconbtn rc-edit" data-id="${r.id}">✏️</button>
          <button class="iconbtn rc-del" data-id="${r.id}">🗑️</button>
        </td>
      </tr>`;
  ...
  `;
        ...
        if (svcType === 'service-rate' || svcType !== 'default') {
            ...
            html += `<div id="rcSvcList" style="max-height:300px;overflow:auto;border:1px solid #eee;padding:8px;border-radius:6px">${'<div style="color:#777">Loading…</div>'}</div>`;
            if (mode !== 'view') {
                html += `<div style="margin-top:8px"><button class="btn" id="rcAddSvcBtn">＋ Add Service Rate</button></div>`;
            }
        }
        ...
        function renderSvcEditor() {
          ...
          <div class="rcFormRow"><label>Category</label><div class="rcValue">${esc(s.category || '—')}</div></div>
          <div class="rcFormRow"><label>Rate</label><div class="rcValue">${esc(String(s.rate_value ?? '—'))} ${s.currency ? esc(s.currency) : ''}</div></div>
          <div class="rcFormRow"><label>Type</label><div class="rcValue">${esc(s.rate_type || '—')}</div></div>
          <div class="rcFormRow"><label>Region</label><div class="rcValue">${esc(s.region || '—')}</div></div>
          ...
          <div class="rcFormRow">
            ...
            <div style="margin-left:auto;display:flex;gap:6px">
              ...
              <button class="btn small rcSvcDel" data-sid="${sid}">✖</button>
            </div>
          </div>
    ...
    container.addEventListener('click', e => {
        ...
        if (e.target.classList.contains('rc-view')) {
            ...
            <div>${(d.service_rates || []).map(s => `<div style="padding:6px;border-bottom:1px solid #eee"><b>${esc(s.category)}</b> — ${esc(s.region)} — ${esc(s.rate_type)} ${s.rate_value}</div>`).join('')}</div>
          `;
                    modalBackdrop.style.display = 'flex';
